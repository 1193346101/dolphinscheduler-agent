...
class PythonSkill(BaseSkill):
    ...
    def _get_scripts_dir(self) -> Optional[Path]:
        """获取 python-error-analyzer scripts 目录"""
        scripts_dir = Path(__file__).parent / "python-error-analyzer" / "scripts"
        if scripts_dir.exists():
            return scripts_dir
        scripts_dir = Path(__file__).parent / "python_error_analyzer" / "scripts"
        if scripts_dir.exists():
            return scripts_dir
        return None

    def _get_patterns_file(self) -> Optional[Path]:
        """获取 python_patterns.md 文件路径"""
        skill_dir = Path(__file__).parent / "python-error-analyzer"
        patterns_file = skill_dir / "python_patterns.md"
